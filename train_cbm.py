...
parser.add_argument("--acc", type=str, default=None, help="set to json file if want to apply acc")
...
def train_cbm_and_save(args):
    ...
    with torch.no_grad():
        ...
        if args.acc is not None:
            image_class_indices = data_utils.get_targets_only(d_train)
            val_image_class_indices = data_utils.get_targets_only(d_val)
            ...
            clip_features = torch.where(mask, clip_features, torch.tensor(0.0))

            num_images, _ = val_clip_features.shape
            mask = torch.zeros((num_images, num_concepts), dtype=torch.bool)
            class_indices_tensor = torch.tensor(val_image_class_indices)
            mask = concept_class_membership[:, class_indices_tensor].T
            val_clip_features = torch.where(mask, val_clip_features, torch.tensor(0.0))
            print("ACC:finished setting incorrect concept to zero")
    ...
    for i in range(args.proj_steps):
        ...
        if i%50==0 or i==args.proj_steps-1:
            ...
            if i==0:
                ...
                break
            print("Step:{}, Avg train similarity:{:.4f}, Avg val similarity:{:.4f}".format(best_step, -loss.cpu(),-best_val_loss.cpu()))
        opt.zero_grad()
