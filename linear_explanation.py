...
#     hooks = {}
#     for target_layer in target_layers:
#         command = f"target_model.{target_layer}.register_forward_hook(get_activation_slice(all_features[target_layer], pool_mode, start, end))"
#         hooks[target_layer] = eval(command)
...
#     with torch.no_grad():
#         for i, (images, labels) in enumerate(tqdm(DataLoader(dataset, batch_size, num_workers=8, pin_memory=True))):
...
#                     print(f"Model output is a tuple with {len(output)} elements")
...
#     for target_layer in target_layers:
#         hooks[target_layer].remove()
#         print(f"Collected features for layer {target_layer}: {len(all_features[target_layer])} batches")
    ...
    with torch.no_grad():
        ...
        for i, (images, labels) in enumerate(DataLoader(dataset, batch_size=batch_size, num_workers=8, pin_memory=True)):
            with torch.no_grad():
                ...
                # for lf_cbm only
                if model.__class__.__name__=='CBM_model':
                    outs_no_calib=outs_no_calib[0]
...
def save_pal_without_neuron(model, dataset, device, target_layer, target_neuron, save_path, batch_size=128, T=1):
    if "encoder.layers" in target_layer:
        ...
        def new_forward(self, x):
            ...
            for i, block in enumerate(self.encoder.layers):
                if target_layer.startswith(f"encoder.layers.encoder_layer_{i}"):
                    # assume mlp[1]
                    # x is input, x2 is x, 
                    torch._assert(x.dim() == 3, f"Expected (batch_size, seq_length, hidden_dim) got {x.shape}")
                    x2 = block.ln_1(x)
                    x2, _ = block.self_attention(x2, x2, x2, need_weights=False)
                    x2 = block.dropout(x2)
                    x2 = x2 + x
...
#                     y[:, :, target_neuron] *= 0
        ...
        def new_forward(self, x):
            ...
            # Zero out the target neuron in the projected concepts
            x[:, target_neuron] *= 0
        ...
        try:
            result = save_pred_acc_loss(model, dataset, device, save_path, batch_size=batch_size, T=T)
        finally:
            # Ensure the original forward method is restored even if an exception occurs
